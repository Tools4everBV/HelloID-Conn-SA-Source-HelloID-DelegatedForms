...
#HelloID variables
$script:PortalBaseUrl = "https://<klant>.helloid.com"
$apiKey = "<apikey>"
$apiSecret = "<apisecret>"
...
function Get-HelloIDData([string]$endpointUri) {
    ...
    while ($paged) {
        ...
        $response = (Invoke-RestMethod -Method GET -Uri $uri -Headers $script:headers -ContentType 'application/json')
...
foreach ($form in $allForms) { 
    $delegatedForm = (Get-HelloIDData -endpointUri "/api/v1/delegatedforms/$($form.delegatedFormGuid)")
    $delegatedFormName = $delegatedForm.Name
